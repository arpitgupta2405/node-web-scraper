...
WebScrapper.prototype.getAllHyperlinks = async function(url) {
 //async function getAllHyperlinks(url) {
  ...
  https.get(url, (resp) => {
   ...
   resp.on('end', () => {
    while ((match = regexp.exec(html)) != null) {
     ...
     if (url.indexOf('xml') === -1 && url.indexOf('cdn-') === -1 && url.indexOf('https://') > -1 && url.indexOf('android-app') === -1 && url.indexOf('js') === -1 && url.indexOf('.css') === -1 && url.indexOf('medium.com') > -1) {
      if (url.indexOf('?') > -1) {
 ...
 (async function loop() {
  for (let item of globleurls) {
   if (item.indexOf('https://') > -1) {
    var p = await self.getAllHyperlinks(item);
    p.forEach((r) => {
     if (!globleurls.has(r)) {
